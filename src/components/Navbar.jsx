...
export default function Navbar() {
  ...
  return (
    <>
  <AppBar position="fixed" color="transparent" elevation={0} sx={{ zIndex: 1201, width: '100vw', left: 0, top: 0, backdropFilter: 'blur(6px)', backgroundColor: 'rgba(24,28,34,0.60)' }}>
        <Toolbar sx={{ gap: 2, minHeight: { xs:0, md:0 }, paddingTop:2 }}>
          <IconButton color="inherit" edge="start" sx={{ display: { xs: 'inline-flex', md: 'none' } }} onClick={toggleDrawer} aria-label="open navigation">
            <MenuIcon />
          </IconButton>
          <Typography variant="h6" component={Link} to="/" sx={{ flexGrow: { xs: 1, md: 0 }, textDecoration: 'none', color: 'white', fontWeight: 800, letterSpacing: 1 }}>
            M.V.V BUILDERS
          </Typography>
          <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: 1, ml: 'auto', alignItems: 'center' }}>
            {navItems.map((item) => (
              <Button key={item.to} component={Link} to={item.to} color="inherit" sx={{ fontWeight: 600, letterSpacing: 0.5, color: 'white' }}>
  ...
  <Toolbar sx={{ minHeight: { xs:0, md:0 }, paddingTop:2, background: 'transparent' }} />
